...
const printSucces = (title: string, message: string) => {
  Swal.fire({
    icon: "success",
    title: title,
    text: `${message}`,
...
export const editNews = async (
  ...
  try {
    ...
    if (!validateNewsData(data, img, video)) {
      ...
      setLoading(false);
      printSucces("Editada", "Noticia editada correctamente");
      return true;
    }
  } catch (error) {
    setLoading(false);
...
export const uploadNews = async (
  ...
  try {
    ...
    if (!validateNewsData(data, img, video)) {
      ...
      setLoading(false);
      printSucces("Añadida", "Noticia añadida correctamente");
      return true;
    }
  } catch (error) {
    setLoading(false);
  ...
  deleteObject(imgRef)
    .then(() => {
      deleteObject(videoRef).then(() => {
        ...
        printSucces("Eliminada", "Noticia eliminada correctamente");
      });
    })
    .catch(() => {
      printError("Ha ocurrido un error durante la transacción!");
